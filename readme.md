...
If you provide a Gemini API key via the gemini-api-key attribute, an "insight" button appears on the answer side of cards to render a concise, table-style analysis of the sentence.

-   Enable by setting gemini-api-key="YOUR_KEY" on material-beautify-chinese-study.
-   Omit the attribute (or set it empty) to disable insights — nothing is rendered and no calls are made.

### Security & Risk Note:

The API key lives only on your device in the Anki card webview. For anyone else to access it:

-   Your device itself would have to be compromised (e.g., malware, physical access).
-   You would also likely need to be on an insecure network, such as public Wi-Fi, where attackers have an easier time intercepting or injecting traffic.

Using mobile data (4G/5G) is considered secure because cellular networks encrypt and isolate your traffic. The primary risks come from public Wi-Fi or a compromised device, not from normal carrier data connections.

If you also **do not attach billing information** to your Gemini account, the risk is minimal:

-   The worst-case scenario is someone using up your free quota, which could temporarily prevent you from using the service until it resets.
-   No financial charges can occur without a billing method on file.
...
1. Phonics Autogeneration + Overrides

    - As long as a 'primary' character is present and you do not to supply phonic values, this can be auto‑generated.
        - `preferred-phonic="pinyin"` (default) generates accented pinyin
        - `preferred-phonic="zhuyin"` generates bopomofo (ㄅㄆㄇㄈ)
    - You may provide your own numbered pinyin via `numbered-pinyin` and/or `sentence-numbered-pinyin`.
        - Tokens may be comma‑ or space‑separated. If a token ends with a letter, a neutral tone 5 is appended for robustness.
    - Set `force-auto-generation` to ignore any provided pinyin and always compute from characters.
...
1. Stroke Order Animation (Writing)
    - On the answer side of writing cards, the hanzi animates stroke order by default.
...
Feel free see to this project in action using this [jsFiddle](https://jsfiddle.net/loganconnor44/Lnu9jm0f/166/).
