...
const conditionallyRenderColourSchemes = (questionType) => {
    let body = document.body;
    let card = document.getElementsByClassName('chinese-card').item(0);
    let cardType = document.getElementsByClassName('chinese-card-type').item(0);
    let cardContent = document.getElementsByClassName('chinese-card-content').item(0);
    switch (questionType) {
        case 'traditional':
            var darkest = '#264653';
            var darker = '#2A9D8F';
...
const createStrokeOrderCharacter = () => {
    ...
    let charactersValue = document.getElementById('character-value').innerHTML;
    let characters = charactersValue.split('');
    let drawingArea = document.getElementById('stroke-order');
    ...
    for (var i = 0; i < characters.length; i++) {
        ...
        let strokeOrderCharacter = HanziWriter.create(idName, characters[i], {
            strokeColor: '#000000',
            width: 50,
            height: 50,
            padding: 1,
    ...
    const animateFirstCharacterOnly = () => {
        for (var i = 0; i < 1; i++) {
            let currentCharacter = strokeOrderCharacters[i];
            let nextCharacter = strokeOrderCharacters[i + 1];
